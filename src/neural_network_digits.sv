...
module neural_network_digits (
 ...
 output logic [3:0] digit
  ...
  localparam int N_FMA = 16; // Numero de FMAs en cadena (16 = 64/4, 64 pesos por neurona, 4 pasadas)
  ...
  // Contadores
  logic [1:0] bloque_fma_cuenta;    // Contador para reutilizar los 16 FMA durante la "hidden layer" se necesitan 4 pasadas por estos 16 FMAs
  logic [3:0] contador_neuronas_l1;    // Neuronas de la capa 1 
  logic [3:0] contador_neuronas_l2;   // Neuronas de la capa 2
  logic [1:0] bloque_fma_cuenta_next;
  ...
  logic acc_init;
  logic signed [31:0] fma_result;   // Resultado final del ultimo FMA
  ...
  assign last_p1 = (bloque_fma_cuenta  == 2'd3)  ? 1'b1 : 1'b0;  // última pasada por los 16 FMA de la capa 1
  assign last_n1 = (contador_neuronas_l1  == 4'd15) ? 1'b1 : 1'b0;  // última neurona L1
  assign last_n2 = (contador_neuronas_l2 == 4'd9)  ? 1'b1 : 1'b0;  // última neurona L2
  ...
  // Contador para saber cuantas veces se ha usado el bloque de 16 FMAs en la capa 1
  assign bloque_fma_cuenta_next = weights_layer1_rd_en ? (bloque_fma_cuenta + 2'd1) : 2'd0;

  always_ff @(posedge clk or posedge rst) begin
    if (rst) bloque_fma_cuenta <= 2'd0;
    else     bloque_fma_cuenta <= bloque_fma_cuenta_next;
  end
  ...
  // Instanciación de los 16 FMAs en cadena
  // FMA 0 en la primer iteración entra como suma el BIAS y en las siguientes iteraciones entra el resultado de un FMA pasado
  generate
    for (k = 0; k < N_FMA; k++) begin : gen_bloque_fma
      assign w_data[k] = w_data_packed[k*8 +: 8];  // Asignar 8 bits de cada peso a cada FMA
      fma_unit u_fma (
        .a    (fma_srca_mux[k]),          // uint8 (pixel zero-ext o activación)
        .b    ($signed(w_data[k])),       // int8  (peso)
        .c_in (bloque_fma[k]),            // int32 (resultado FMA anterior)
        .out  (bloque_fma[k+1])           // int32 (hacia FMA siguiente)
      );
    end
  endgenerate

  assign fma_result = bloque_fma[N_FMA];        // Resultado del ultimo FMA (en este caso el numero 16)

  // Registro del acumulador: se actualiza solo en los estados Layer 1 o Layer 2
  always_ff @(posedge clk or posedge rst) begin
    if (rst)         acumulador <= 32'd0;
    else if (weights_layer1_rd_en | weights_layer2_rd_en) acumulador <= fma_result;
  end

  // contador_neuronas_l1 (va de 0 a 15)
  always_ff @(posedge clk or posedge rst) begin
    if (rst)
      contador_neuronas_l1 <= 4'd0;
    else if (weights_layer1_rd_en && bloque_fma_cuenta == 2'd3)
      contador_neuronas_l1 <= (contador_neuronas_l1 == 4'd15) ? 4'd0 : contador_neuronas_l1 + 4'd1;
    else if (!weights_layer1_rd_en)
      contador_neuronas_l1 <= 4'd0;
  end

  // contador_neuronas_l2 (va de 0 a 9)
  always_ff @(posedge clk or posedge rst) begin
    if (rst)
  ...
  // La recuantificacion se realiza al final de la capa 1
  assign recuantificacion = weights_layer1_rd_en && (bloque_fma_cuenta == 2'd3);
  ...
  always_ff @(posedge clk) begin
    if (weights_layer2_rd_en)
      neuronas_capa2[contador_neuronas_l2] <= fma_result;
  end

  // Comparacion entre neuronas capa 2 para ver cual tiene el mayor valor
  always_comb begin
    argmax_neuronas_l2 = 4'd0;                       
    for (int i = 1; i < 10; i++) begin
      if (neuronas_capa2[i] > neuronas_capa2[argmax_neuronas_l2])
        argmax_neuronas_l2 = 4'(i);                   // actualizar si encontramos un score mayor
    end
  end

  assign digit = argmax_neuronas_l2;
